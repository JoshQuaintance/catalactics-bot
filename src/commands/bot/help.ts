...
export const help: CommandsType = {
  ...
  command: async function getAllCommands(msg, { settings, CommandList }) {
    try {
      ...
      // Gets the argument
      const splitMsg = msg.content.split(' ')[1];

      // Finds the command using the prefix and return the object.
      const commandFound = CommandList.find((command: CommandsType) => command.prefix == splitMsg);
      ...
      if (splitMsg == undefined || commandFound == undefined) {
        ...
        const title = commandFound.additionalParam
        ...
        await Data.findOne({ prefix: commandFound.prefix }, (err: never, data: CommandsDbInt) => {
          if (err) throw err;
