 ...
 */
function info(msg: Message, role: string) {
	...
	Role.findOne({ roleId: roleId }, (err: any, data: RoleType) => {
		...
		if (data.desc == 'No Description Added') {
			let embed = newMsg(
				'#f96e46',
				'Role Info not Available',
				`I'm sorry ${msg.author}, the info on ${role} is not available. The owner of the server did not add any description about the role.`
			);
			msg.channel.send(embed);
		}
	});
